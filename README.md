...
A fully synthetic preview archive lives at
`test-fixtures/demo-archive/`. It is generated by
`python3 tools/generate_demo_archive.py` and contains fictional chats,
contacts, placeholder media, PDFs, status/story rows, and light/dark wallpaper.
It is safe to use for app previews because it contains no real WhatsApp data.

To preview the app without private data, build the viewer and select the
`test-fixtures/demo-archive/` folder. Selecting the folder, rather than the
SQLite file alone, lets the app resolve demo media and wallpaper files.
