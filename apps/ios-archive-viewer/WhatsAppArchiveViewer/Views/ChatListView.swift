        ...
        NavigationStack {
            List {
                Section {
                    ...
                    Text("Default keeps the selected archive's existing wallpaper files, including the bundled demo wallpaper. Other choices are generated by the app and do not edit or copy archive files.")
                }
            }
...
private struct WallpaperThemeThumbnail: View {
    ...
    private var thumbnailPattern: some View {
        ZStack {
            ForEach(0..<6, id: \.self) { index in
                Circle()
                    .stroke(patternColor.opacity(0.24), lineWidth: 1)
                    .frame(width: CGFloat(9 + index % 3 * 4), height: CGFloat(9 + index % 3 * 4))
                    .offset(
                        x: CGFloat((index % 3) * 18 - 18),
                        y: CGFloat((index / 3) * 18 - 9)
                    )
            }

            ForEach(0..<5, id: \.self) { index in
                Rectangle()
                    .fill(patternColor.opacity(0.16))
                    .frame(width: 34, height: 1)
                    .rotationEffect(.degrees(-24))
                    .offset(x: CGFloat(index * 14 - 28), y: CGFloat(index % 2 == 0 ? -10 : 10))
            }
        }
    ...
    private var patternColor: Color {
        switch theme {
        case .classic:
            return colorScheme == .dark ? Color(red: 0.74, green: 0.88, blue: 0.80) : Color(red: 0.24, green: 0.42, blue: 0.34)
        case .softPattern:
            return colorScheme == .dark ? Color(red: 0.70, green: 0.75, blue: 0.78) : Color(red: 0.36, green: 0.43, blue: 0.48)
        case .demo:
            return colorScheme == .dark ? Color(red: 0.78, green: 0.72, blue: 0.60) : Color(red: 0.50, green: 0.40, blue: 0.30)
        default:
            return .secondary
        }
    }
