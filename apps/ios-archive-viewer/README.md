...
- Supports a local chat wallpaper selector. Default uses extracted archive wallpaper files when present, while Classic, Soft Pattern, Demo, and Plain are generated by the app.
...
- Shows the selected wallpaper style behind messages. Default uses `current_wallpaper.jpg` or `current_wallpaper_dark.jpg` next to `ChatStorage.sqlite` when available.
...
- Chat wallpaper resolution checks generic archive-root files named `current_wallpaper.jpg` and `current_wallpaper_dark.jpg`; dark mode prefers the dark companion when both are present. The wallpaper selector is local app metadata and does not edit archive files. Built-in Classic, Soft Pattern, and Demo styles are generated by the app, and Plain disables wallpaper.
